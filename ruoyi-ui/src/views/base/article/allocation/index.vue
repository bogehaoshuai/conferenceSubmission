  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-plus"
          size="mini"
          @click="handleRebuttal"
          v-if="articleStatusCode === 3"
          >rebuttal</el-button
        >
      </el-col>
      <!--      <el-col :span="1.5">-->
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.reviewStatusCode === 0"
            size="mini"
            type="text"
            icon="el-icon-add"
            @click="handleUpdate(scope.row)"
            v-hasPermi="['review:allocation:edit']"
            >审核</el-button
          >
          <el-button
            size="mini"
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <!-- <el-form-item label="录用标签" prop="acceptCode">
          <el-select v-model="form.acceptCode" :disabled="true">
            <el-option
...
import {
  delAllocation,
  getAllocation,
  listAllocationByArticleId,
} from "@/api/base/allocation";
import { getArticle } from "@/api/base/article";
import {
  addRebuttal,
  listRebuttalByArticleId,
} from "../../../../api/base/rebuttal";

export default {
  name: "Allocation",
  dicts: ["review_status_code", "confidence_code", "score_code", "accept_code"],
  data() {
    return {
      defaultArticleId: "",
      defaultArticleName: "",
  ...
  methods: {
    getArticle(articleId) {
      getArticle(articleId).then((response) => {
        ...
        this.defaultArticleId = articleId;
    ...
    getList() {
      this.loading = true;
      listAllocationByArticleId(this.queryParams.articleId).then((response) => {
        this.allocationList = response.rows;
        this.total = response.total;
        this.loading = false;
      });
    ...
    getList1(articleId) {
      this.loading = true;
      listAllocationByArticleId(articleId).then((response) => {
        this.allocationList = response.rows;
        this.total = response.total;
        this.loading = false;
      });
    ...
    handleRebuttal() {
      this.reset();
      this.title = "rebuttal信息";
      listRebuttalByArticleId(this.queryParams.articleId).then((response) => {
        if (response.rows.length !== 0) {
          this.form = response.rows[0];
          this.rebuttalExisted = true;
        }
        this.open = true;
        this.form.articleName = this.defaultArticleName;
        this.form.articleId = this.defaultArticleId;
      });
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          addRebuttal(this.form).then((response) => {
            this.$modal.msgSuccess("新增成功");
            this.open = false;
            // this.getList(this.queryParams.articleId);
            this.getList();
          });
        }
      });
